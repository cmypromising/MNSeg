...
class nnUNetLogger(object):
    ...
    def plot_progress_png(self, output_folder):
        ...
        #          linewidth=4)
        # ax2.plot(x_values, self.my_fantastic_logging['mean_fg_iou'][:epoch + 1], color='y', ls='-', label="fiber iou", linewidth=4)
        # 绘制新的平均指标  
        ax2.plot(x_values, self.my_fantastic_logging['mean_dice'][:epoch + 1], color='b', ls='-', label="Mean Dice", linewidth=4)  
        ax2.plot(x_values, self.my_fantastic_logging['mean_iou'][:epoch + 1], color='r', ls='-', label="Mean IoU", linewidth=4)  
        # ax.plot(x_values, self.my_fantastic_logging['mean_precision'][:epoch + 1], color='g', ls='-', label="Mean Precision", linewidth=4)  
        # ax.plot(x_values, self.my_fantastic_logging['mean_recall'][:epoch + 1], color='y', ls='-', label="Mean Recall", linewidth=4)  
        ...
        ax.set_xlabel("epoch")
        ax.set_ylabel("loss")
        ax2.set_ylabel("pseudo dice / fiber iou")
        ax.legend(loc=(0, 1))
        ax2.legend(loc=(0.2, 1))

        # precision and recall
        ax = ax_all[1]
        ax.plot(x_values, self.my_fantastic_logging['mean_precision'][:epoch + 1], color='b', ls='-', label="fiber precision", linewidth=4)
        ax.plot(x_values, self.my_fantastic_logging['mean_recall'][:epoch + 1], color='r', ls='-', label="fiber recall", linewidth=4)
        ax.set_xlabel("epoch")
