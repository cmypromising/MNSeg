...
class nnUNetTrainer(object):
    ...
    def _build_loss(self):
        ...
        weight_cldice=0.5
        weight_cbdice=0
        ce_kwargs={'weight': torch.tensor([1.,5.]).cuda()}
        cldc_kwargs={'iter_': 3}
        cbdc_kwargs={'iter_': 3}
        if self.label_manager.has_regions:
            ...
            print("using DC_and_CE_and_clDC_loss...")
            loss = DC_and_CE_and_CLDC_loss({'batch_dice': self.configuration_manager.batch_dice,
                                   'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, ce_kwargs, cldc_kwargs,
                                  weight_ce=weight_ce, weight_dice=weight_dice, weight_cldice=weight_cldice,
    ...
    def on_train_start(self):
        if not self.was_initialized:
            self.initialize()
        # self.test_random_seed()
        param = self._count_parameters_in_M(self.network)
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        del tp, fp, fn, output, predicted_segmentation_onehot, target
        loss_value = l.detach().cpu().numpy()
        del l

        return {'loss': loss_value, 'tp_hard': tp_hard, 'fp_hard': fp_hard, 'fn_hard': fn_hard}
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        # 计算每个类别（包括背景）的指标  
        dice_per_class = [2 * i / (2 * i + j + k) if (2 * i + j + k) > 0 else np.nan for i, j, k in zip(tp, fp, fn)]  
        mean_dice = np.nanmean(dice_per_class)  

        # 计算每个类别（包括背景）的IoU  
        iou_per_class = [i / (i + j + k) if (i + j + k) > 0 else np.nan for i, j, k in zip(tp, fp, fn)]  
        mean_iou = np.nanmean(iou_per_class)  

        # 计算每个类别（包括背景）的Precision  
        precision_per_class = [i / (i + j) if (i + j) > 0 else np.nan for i, j in zip(tp, fp)]  
        mean_precision = np.nanmean(precision_per_class)  

        # 计算每个类别（包括背景）的Recall  
        recall_per_class = [i / (i + k) if (i + k) > 0 else np.nan for i, k in zip(tp, fn)]  
        mean_recall = np.nanmean(recall_per_class)  

        # 记录每个类别的指标  
        self.logger.log('dice_per_class', dice_per_class, self.current_epoch)  
        self.logger.log('iou_per_class', iou_per_class, self.current_epoch)  
        self.logger.log('precision_per_class', precision_per_class, self.current_epoch)  
        self.logger.log('recall_per_class', recall_per_class, self.current_epoch)  

        self.logger.log('mean_fg_dice', dice_per_class[-1], self.current_epoch)

        # 记录平均指标  
        self.logger.log('mean_dice', mean_dice, self.current_epoch)  
        self.logger.log('mean_iou', mean_iou, self.current_epoch)  
    ...
    def on_epoch_end(self):
        ...
        # 使用全类别（包括背景）的平均指标  
        self.print_to_log_file('mean_dice', np.round(self.logger.my_fantastic_logging['mean_dice'][-1], decimals=4))  
        self.print_to_log_file('mean_iou', np.round(self.logger.my_fantastic_logging['mean_iou'][-1], decimals=4))  
        self.print_to_log_file('mean_precision', np.round(self.logger.my_fantastic_logging['mean_precision'][-1], decimals=4))  
        self.print_to_log_file('mean_recall', np.round(self.logger.my_fantastic_logging['mean_recall'][-1], decimals=4))  
        
        # 打印所有类别的Dice指标  
        self.print_to_log_file('Dice per class', [np.round(i, decimals=4) for i in   
                                                self.logger.my_fantastic_logging['dice_per_class'][-1]])  
        self.print_to_log_file('IOU per class', [np.round(i, decimals=4) for i in   
                                                self.logger.my_fantastic_logging['iou_per_class'][-1]])
        self.print_to_log_file('precision per class', [np.round(i, decimals=4) for i in   
                                                self.logger.my_fantastic_logging['precision_per_class'][-1]])
        self.print_to_log_file('recall per class', [np.round(i, decimals=4) for i in   
                                                self.logger.my_fantastic_logging['recall_per_class'][-1]])
    ...
    def test_random_seed(self):
        ...
        # 生成 Python random 模块的随机数并打印
        random_number = random.random()
        print("\nRandom Number (Python random):")
        print(random_number)
        # input_tensor = torch.randn((1, 10, 32, 128, 128)).to(device)
        # # 实例化模型并应用初始化
        # model = self.network()
        #
        # # 确保模型在正确的设备上
