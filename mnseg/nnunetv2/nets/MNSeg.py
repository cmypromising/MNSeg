...
class Cov3x3_BN(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
                 padding=1, dilation=1, groups=1, bias=True, with_BN=True):
...
class MBA(nn.Module):
    def __init__(self,
        ...
        self.conv = Cov3x3_BN(in_channels = output_channels, out_channels = 1, kernel_size = 1, stride=1, padding = 0, with_BN = True)
        self.conv_f = Cov3x3_BN(in_channels = output_channels, out_channels = output_channels, kernel_size = 1, padding = 0, with_BN = True)
        self.conv_rf = Cov3x3_BN(in_channels = output_channels, out_channels = output_channels, kernel_size = 1, padding = 0, with_BN = True)
        self.mamba_layer = MambaLayer(dim=1)

    def forward(self, f1, f2):
        attention = F.sigmoid(self.mamba_layer(self.conv(f2)))
        r_attention = 1 - attention
        f1_a = self.conv_f(f1 * attention)
        f1_ra = self.conv_rf(f1 * r_attention)
        return f1 + f1_a + f1_ra
...
class UNetResDecoder(nn.Module):
    def __init__(self,
        ...
        seg_layers = []
        print(n_stages_encoder)
        for s in range(1, n_stages_encoder):
            input_features_below = encoder.output_channels[-s]
            input_features_skip = encoder.output_channels[-(s + 1)]
            stride_for_upsampling = encoder.strides[-s]

            eam_layers.append(EAM(
        ...
        self.eam_layers = nn.ModuleList(eam_layers)
    ...
    def forward(self, skips):
        ...
        for s in range(len(self.stages)):
            ...
            f2 = self.upsample_layers[s](skips[-(s+1)])
            eam = self.eam_layers[s](skips[-(s+2)], f2)
...
def get_mnseg_3d_from_plans(
    ...
    segmentation_network_class_name = 'MNSeg'
    network_class = MNSeg
    kwargs = {
        'MNSeg': {
            'conv_bias': True,
